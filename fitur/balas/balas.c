...
int BalasKicauan(ListDinKicauan lk, int IDKicauan, int IDBalasan, currentUser CU, ListUserStatik *LU, ListTree *lt, Graph graph)
{
    ...
    { // Jika sudah maka mencari ListDinBalasan dengan IDKicauan lalu insert balasan baru
        ...
        TreeNode *balasanNode = createNodeWithData(BALASAN_NODE, &newBalasan);
        if (IDBalasan == -1)
        { // Jika IDBalsan -1 maka membalas root parent makan menambah langsung ke root
            addChild(kicauanNode, balasanNode);
        }
        else
        { // Jika membalasan ke Balasan maka akan mendapatkan Node IDKicauan dan searching Node IDBalasan untuk ditambahkan
    ...
    printf("| ");
    printWord(AuthorBalasan(newBalasan));
    printf("\n");
    printf("| ");
    TulisDATETIME(DatetimeBalasan(newBalasan));
    printf("\n");
    printf("| ");
    printWord(TxtBalasan(newBalasan));
    printf("\n");
    printf("\n");
...
void DisplayBalasanRecursive(TreeNode *T, int level, ListUserStatik LU, int idCurrentUser, Graph graph)
{
    ...
    while (current != NULL)
    {
        ...
        if (isTeman(&graph, idCurrentUser, idUserBalasan) == false && JENIS_USER(LU, idUserBalasan) == 0 && idCurrentUser != idUserBalasan)
        { // Jika current user dan user balasan bukan teman dan user balasan privat maka PRIVATTTT SEMUAAA
        ...
        else
        {
            ...
            printf("| ");
            TulisDATETIME(DatetimeBalasan(B));
            printf("\n");
            for (int i = 0; i < level; i++)
            {
                printf("\t");
